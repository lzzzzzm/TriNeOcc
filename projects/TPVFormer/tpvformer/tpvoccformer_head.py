...
class TPVOccFormerDecoder(BaseModule):
    ...
    def predict(self, tpv_list, batch_data_samples):
        ...
        for i, data_sample in enumerate(batch_data_samples):
            ...
            tpv_hw_vox = tpv_hw[i:i+1].unsqueeze(-1).permute(0, 1, 3, 2, 4).expand(-1, -1, -1, -1,
                                                                                   self.scale_z*self.tpv_z)
            tpv_zh_vox = tpv_zh[i:i + 1].unsqueeze(-1).permute(0, 1, 4, 3, 2).expand(-1, -1, self.scale_w * self.tpv_w, -1,
                                                                                     -1)
            tpv_wz_vox = tpv_wz[i:i + 1].unsqueeze(-1).permute(0, 1, 2, 4, 3).expand(-1, -1, -1, self.scale_h * self.tpv_h,
                                                                                     -1)
            # 2D predictor
            fused_vox = (tpv_hw_vox + tpv_zh_vox + tpv_wz_vox).flatten(2).squeeze(0).transpose(0, 1)
            fused = self.decoder(fused_vox)
            logit = self.classifier(fused)
            logits.append(logit)

        return logits

    def loss(self, tpv_list, batch_data_samples, use_camera_mask=False):
        tpv_hw, tpv_zh, tpv_wz = tpv_list
        bs, _, c = tpv_hw.shape
        tpv_hw = tpv_hw.permute(0, 2, 1).reshape(bs, c, self.tpv_h, self.tpv_w)
        tpv_zh = tpv_zh.permute(0, 2, 1).reshape(bs, c, self.tpv_z, self.tpv_h)
        ...
        batch_vox = torch.cat(batch_vox, dim=0)
        logits = self.decoder(batch_vox)
        logits = self.classifier(logits)

        occ_seg_label = torch.cat([
            data_sample.gt_occ_seg.occ_semantics.flatten().to(torch.int64)
            for data_sample in batch_data_samples
        ])
        occ_camera_mask = torch.cat([
            data_sample.gt_occ_seg.occ_mask_camera.flatten().to(torch.bool)
            for data_sample in batch_data_samples
        ])
        if use_camera_mask:
            not_occ_camera_mask = torch.logical_not(occ_camera_mask)
            occ_seg_label[not_occ_camera_mask] = self.ignore_index
            logits[not_occ_camera_mask] = self.ignore_index

        lovasz_input = ce_input = logits
        lovasz_label = ce_label = occ_seg_label.to(torch.int64)
